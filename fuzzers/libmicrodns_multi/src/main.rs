...
use libafl::{
    bolts::{
        current_nanos, rands::StdRand, tuples::tuple_list,
...
fn main() {
    ...
    let cores = parse_core_bind_arg("all").unwrap();
    let stats = MultiStats::new(|s| println!("{}", s));
    let shmem_provider = StdShMemProvider::new().unwrap();

    let mut run_client = |_: Option<StdState<_, _, _, _, _>>, mut restarting_mgr| {
        // 32Mb of memory
        ...
        state
            .generate_initial_inputs(&mut fuzzer, &mut executor, &mut generator, &mut restarting_mgr, 64)
        ...
        fuzzer
            .fuzz_loop(&mut stages, &mut executor, &mut state, &mut restarting_mgr)
